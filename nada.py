...
class YAIverseGAN(torch.nn.Module):
    ...
    def __init__(self, engine: "Engine|dict|str"):
    ...
    def determine_opt_layers(self):
        ...
        for _ in range(self.auto_layer_iters):
            generated_from_w, _ = generate_custom(self.generator_trainable, tex_z=w_tex_codes, geo_z=w_geo_codes,
                                                  mode='layer')  # (B, C, H, W)
...
if __name__ == "__main__":
    ...
    ut_gan = YAIverseGAN(args.config_path)

    [img1, img2], loss = ut_gan(
        tex_z=torch.randn(4, 512, device=args.device),
        geo_z=torch.randn(4, 512, device=args.device)
    )
