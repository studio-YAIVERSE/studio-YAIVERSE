...
def generate_custom_synthesis(self:DMTETSynthesisNetwork, ws, ws_geo, camera=None, texture_resolution=2048, mode='nada',
                       **block_kwargs):
        """
...
def addon(instance:GeneratorDMTETMesh):

    if not hasattr(instance.synthesis, 'generate_custom') :
        print("ADD ON : DMTETSynthesisNetwork.generate_custom")
        instance.synthesis.generate_custom = types.MethodType(generate_custom_synthesis, instance.synthesis)

    if not hasattr(instance, 'generate_custom') :
        print("ADD ON : GeneratorDMTETMesh.generate_custom")
        instance.generate_custom = types.MethodType(generate_custom, instance)

    if not hasattr(instance, 'get_all_generator_layers_dict') :
        print("ADD ON : get_all_generator_layers_dict")
        instance.get_all_generator_layers_dict = types.MethodType(get_all_generator_layers_dict, instance)

    if not hasattr(instance, 'freeze_generator_layers') :
        print("ADD ON : freeze_generator_layers")
        instance.freeze_generator_layers = types.MethodType(freeze_generator_layers, instance)

    if not hasattr(instance, 'unfreeze_generator_layers') :
        print("ADD ON : unfreeze_generator_layers")
        instance.unfreeze_generator_layers = types.MethodType(unfreeze_generator_layers, instance)

    return instance

if __name__ == '__main__' :
    
    model = addon(GeneratorDMTETMesh)
